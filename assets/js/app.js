...
if (recipeSlug) {
  ...
  async function test(recipe, element, quantity) {
    ...
    for (const requirement of recipe.requirements) {
      ...
      const searchRecipe = recipes.find((recipe) => recipe.name === item.name);
  ...
  const recipesContainerEl = document.createElement("div");
  recipesContainerEl.classList.add("card-container");
  ...
  for (const item of recipes) {
    ...
    linkEl.setAttribute("href", `?recipe=${item.slug}&quantity=1`);
    iconEl.setAttribute(
      "src",
      `https://gtcdn.info/vrising${item.iconPath.replace("{height}", 64)}`
    );
    ...
    recipesContainerEl.appendChild(linkEl);
  }

  document.querySelector("#app").appendChild(recipesContainerEl);
}
